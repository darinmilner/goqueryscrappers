...
//Google Scrapper with google domains with goquery
import (
	"fmt"
	"log"
	"math/rand"
...
//googleDomains holds the google domains to be scraped
...
func main() {
	res, err := GoogleScrape("daily news", "com", "en", nil, 1, 30, 10)
	if err == nil {
		for _, res := range res {
			fmt.Print(res)
			//save results to a cv or text file or save to database
		}
	}
}
...
func GoogleScrape(searchTerm, countryCode, languageCode string, proxyString interface{}, pages, count, backoff int) ([]SearchResult, error) {
	...
	for _, page := range googlePages {
		...
		data, err := googleResultParsing(res, resultsCounter)
		if err != nil {
			return nil, err
		}
...
func scrapeClientRequest(searchURL string, proxyString interface{}) (*http.Response, error) {
	...
	if res.StatusCode != 200 {
		err := fmt.Errorf("scrapper got status code %d", res.StatusCode)
		return nil, err
	}
	if err != nil {
		log.Println(err)
